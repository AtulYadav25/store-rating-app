  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    registerUser(
      ...
      },
      {
        ...
        onError: (err: any) => {
          let msg = "Failed to register account. Please check your information.";
          if (err?.response?.data?.message) {
            msg = err.response.data.message;
          } else if (Array.isArray(err?.response?.data?.error)) {
            msg = err.response.data.error.map((e: any) => e.message).join(", ");
      ...
      <div className="w-full max-w-lg space-y-6">
        <div className="flex flex-col items-center text-center space-y-2">
          ...
          <p className="text-sm text-slate-500">
            Join StoreRating to explore and rate local stores
          </p>
        </div>
          ...
          <form onSubmit={handleSubmit}>
            <CardContent className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                {formData.password && (
                  <div className="p-2.5 rounded-lg bg-slate-50 border border-slate-200/80 space-y-1.5 text-xs text-slate-600">
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <Textarea
                    ...
                    disabled={isPending}
                    required
                    className="pl-9 min-h-20 bg-slate-50/50 border-slate-200 focus:bg-white transition-colors resize-none"
                  />
                </div>
              </div>
            </CardContent>
